...
window.InformEnterPlus = {
	...
	init: function(){
		...
		this.menuset.setAttribute("label","设置站点的InformEnter信息");
		...
		cmdmenu.setAttribute("label","设置InformEnter信息");
		...
		//for rulelistmanager
          ...
          <hbox>\
              ...
              <groupbox  flex="1">\
			           <caption label="InformEnterPlus列表" />\
                <tree id="InformEnterPlus-tree" flex="1" seltype="single" onclick="InformEnterPlus.Ruletree.selectRule();">\
            			<treecols>\
            				<treecol id="InformEnterPlus-domain" label="站点" flex="1" crop="center" persist="width ordinal hidden"/>\
                    <splitter class="tree-splitter"/>\
                    <treecol id="InformEnterPlus-menu" label="菜单名称" flex="1" crop="center" persist="width ordinal hidden"/>\
                    <splitter class="tree-splitter"/>\
                    <treecol id="InformEnterPlus-InputID" label="目标的InputID" flex="1" crop="center"  persist="width ordinal hidden"/>\
                    <splitter class="tree-splitter"/>\
                    <treecol id="InformEnterPlus-Value" label="粘帖的内容" hidden="true" persist="width ordinal hidden"/>\
            			 </treecols>\
            			<treechildren/>\
               </tree>\
            	</groupbox>\
              ...
              <vbox id="InformEnterPlus-todo"  height="250" flex="1">\
                  ...
                  <hbox><label value="保存配置才可保存到配置文件! 修改按钮仅对InputID和复制内容有效！"/><spacer flex="1" /><button label="保存配置" id="InformEntersave" oncommand="InformEnterPlus.saveRule();" />\
                              <button label="关闭" id="InformEnterPlus-close" oncommand="InformEnterPlus.Ruletree.closesplicer();" /></hbox>\
                  <hbox><label value="INPUTID可识别框架内的INPUT对象. 所有值不能为空, 站点和Input ID不能有中文！INPUTID可输入*表示任意输入框. "/></hbox>\
                  <hbox><label value="作用站点(域名或主机名,例如sina.com, 全局用*号表示)："/><textbox id="domainvalue" flex="1" /></hbox>\
                  <hbox><label value="菜单名称(弹出菜单标题,不能为空,例如：sina邮箱密码)："/><textbox id="menuvalue" flex="1" /></hbox>\
                  <hbox><label value="InputID(支持ID、NAME、CLASS, 输入*表示任意输入框)："/><textbox id="inputvalue"  flex="1" />\
                  <button label="合并ID" id="InputIDadd" oncommand="InformEnterPlus.Ruletree.addInputID();" />\
                  <button label="移除ID" id="InputIDdel" oncommand="InformEnterPlus.Ruletree.delInputID();" />\
                  ...
                  <hbox><label value="复制内容(弹出菜单自动复制到输入框的内容, 不能为空)："/><textbox id="pastevalue" type="password" flex="1" oncopy="return false;" oncut="return false;"/>\
                  <checkbox id="IMETYPE" checked="false" label="输入法输入" oncommand="InformEnterPlus.setIMEmode(event);"/></hbox>\
                  <hbox>\
				            <button label="添加表单" id="InformEnteradd" oncommand="InformEnterPlus.Ruletree.addlistRule();" />\
				            <button label="删除表单" id="InformEnterdel" disabled="false" oncommand="InformEnterPlus.Ruletree.dellistRule();" />\
				            <button label="修改表单" id="InformEnteredit" disabled="false" oncommand="InformEnterPlus.Ruletree.editlistRule();" />\
				            <spacer flex="1" />\
                  </hbox>\
                  <spacer flex="1" />\
                </vbox>\
	...
	createSubMenu: function() {
		...
		for(var i=0, menu; menu=this.InformArr[i]; ++i)
		{
			...
			if (menu.domainname != "*") {
				  ...
				  menuItem.setAttribute("label", "InformEnter for " + menu.domainname);
			  ...
			  menuItem.setAttribute("label", "InformEnter for 全局");
			  var menuPopup = document.createElement("menupopup");
	...
	addnewmenu: function(isidx, dmn, arec){
        ...
        menuItemtemp.setAttribute("id", dmname + "-ID" + j.toString());
  	    ...
  	    pmenu.appendChild(menuItemtemp);
  		  ...
  		  menuItem.setAttribute("label", "InformEnter for " + dmn);
